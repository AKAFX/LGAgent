        ...
        if self.backend == "vllm":
            ...
            async def call_with_retry(
                ...
                for attempt in range(retries):
                    try:
                        async with sem:
                            app.logger.debug(f"[Generation] Processing question idx={idx} (attempt={attempt+1})")
                            resp = await client.chat.completions.create(
                                model=model_name,
                                messages=msg,
                                **sampling_params,
                            )
                            app.logger.debug(f"[Generation] Completed question idx={idx}")
                        # 检查响应是否有效
                        if resp is None:
                            raise ValueError(f"API returned None response (idx={idx})")
                        if not hasattr(resp, 'choices') or not resp.choices:
                            ...
                            if hasattr(resp, 'error') and resp.error:
                                ...
                                # 余额不足错误：跳过该题目
                                if error_code == '405' or error_type == 'quota_not_enough' or '余额不足' in error_msg:
                                    app.logger.warning(f"[Quota Not Enough] Skipping idx={idx}: {error_msg}")
                                    return idx, "[SKIPPED due to quota not enough]"
                                
                                # 敏感词检测错误：跳过该题目
                                if (error_code == '402' or error_type == 'param_error' or 
                                    error_type == 'data_inspection_failed' or '敏感词' in error_msg):
                                    app.logger.warning(f"[Content Safety] Skipping idx={idx}: {error_msg}")
                                    return idx, "[SKIPPED due to content safety]"
                            
                            raise ValueError(f"API response has no choices (idx={idx}): {resp}")
                        if not resp.choices[0].message:
                            raise ValueError(f"API response has no message (idx={idx})")
                        ...
                        # 检查是否是余额不足错误
                        error_str = str(e)
                        if '405' in error_str or 'quota_not_enough' in error_str or '余额不足' in error_str:
                            app.logger.warning(f"[Quota Not Enough] Skipping idx={idx} (attempt={attempt+1}): {e}")
                            return idx, "[SKIPPED due to quota not enough]"
                        
                        # 检查是否是敏感词检测错误
                        if ('402' in error_str or 'param_error' in error_str or 
                            '敏感词' in error_str or 'data_inspection_failed' in error_str):
                            app.logger.warning(f"[Content Safety] Skipping idx={idx} (attempt={attempt+1}): {e}")
                            return idx, "[SKIPPED due to content safety]"
